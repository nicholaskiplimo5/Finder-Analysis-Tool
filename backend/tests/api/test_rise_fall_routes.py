...
def test_get_rise_fall_summary_skips_symbols_without_data():
    client = make_test_app(
        rise_fall_routes.router,
        rise_fall_service=FakeRiseFallService(skipped_symbols=["R_10"]),
        settings=make_settings(symbols="R_100,R_10"),
    )
    resp = client.get("/api/rise-fall")
...
def test_get_rise_fall_summary_400s_when_every_symbol_skipped():
    client = make_test_app(
        rise_fall_routes.router,
        rise_fall_service=FakeRiseFallService(skipped_symbols=["R_100", "R_10"]),
        settings=make_settings(symbols="R_100,R_10"),
    )
    resp = client.get("/api/rise-fall")
    assert resp.status_code == 400
